...
**Data verification (2026-07-10, project 1084329 via `scripts/inspect_no_code_cost.py`):** of 129,337 lines, 28,715 have no `BASE_MATERIAL_MFC` and 101,724 no `VENDOR_SHOP_FAB_MFC` - and **every one of them carries zero original cost** in the corresponding column. So the rule changes no totals on this project (0 either way); it only becomes visible if a no-code line with cost ever appears. Also observed: the un-prefixed cost columns are typed `NUMBER(18,2)` in the live schema (the `DB_*` twins are strings).
