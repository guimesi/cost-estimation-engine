...
visibly changes).
    ...
    DATA_SOURCE=databricks python scripts/inspect_no_code_cost.py [PLANVIEW_ID]

Read-only. TRY_CAST(... AS DOUBLE) tolerates both the NUMBER(18,2)-typed
un-prefixed cost columns (observed 2026-07-10) and the string-typed DB_*
twins, so the probe works regardless of the migrated column type.
"""
...
def main() -> None:
    ...
    for code_col, cost_col in _PAIRS:
        ...
        df = client.fetch_query(
            ...
            f"SUM(IF({no_code}, 1, 0)) AS NO_CODE_LINES, "
            f"SUM(IF({no_code}, COALESCE(TRY_CAST(c.{cost_col} AS DOUBLE), 0), 0)) "
            f"  AS NO_CODE_COST, "
            f"SUM(IF({no_code} AND COALESCE(TRY_CAST(c.{cost_col} AS DOUBLE), 0) <> 0, "
        ...
        sample = client.fetch_query(
            ...
            f"WHERE {no_code} AND COALESCE(TRY_CAST(c.{cost_col} AS DOUBLE), 0) <> 0"
            f"{scope} LIMIT 5",
            params=params or None,
        )
        if sample.empty:
